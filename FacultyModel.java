...
//		System.out.println("faculty add try block started");
	...
	//		System.out.println("faculty add try block");
			 ...
			 * System.out.println(bean.getCollegeid()); CollegeBean cbean =
			 * cmodel.findByPK(bean.getCollegeid()); bean.setCollegeName(cbean.getName());
			 */
		       
		  
			/*
			 * SubjectModel smodel = new SubjectModel(); SubjectBean sbean =
			 * smodel.findByPk(bean.getSubjectId()); bean.setSubjectName(
			 * sbean.getSubjectName());
			 * 
			 * CourseModel coumodel = new CourseModel(); CourseBean coubean =
			 * coumodel.findByPk(bean.getCourseId()); bean.setCourseName(
			 * coubean.getName());
			 */
	   ...
	   String subjectname = sbean.getSubjectName();
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement(
					"UPDATE ST_FACULTY SET FIRSTNAME=?, LASTNAME=?, GENDER=?, DATEOFJOINING=?, QUALIFICATION=?, LOGINID=?, MOBILENO=? , ADDRESS =?, COLLEGEID=? , COLLEGENAME=?,COURSEID=?,COURSENAME=?, SUBJECTID=?, SUBJECTNAME=?, CREATEDBY=? , MODIFIEDBY=? , CREATEDDATETIME=? , MODIFIEDDATETIME=? WHERE ID=? ");
